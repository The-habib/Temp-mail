...
function BarChart({ messages }: { messages: Msg[] }) {
  ...
  return (
    <svg viewBox="0 0 280 110" className="w-full" xmlns="http://www.w3.org/2000/svg">
      {[0, 4, 8, 12].map((val) => (
        <line
          key={val}
          x1="8"
      ...
      {days.map((day, i) => {
        const barH = counts[i] > 0 ? Math.max((counts[i] / max) * 72, 8) : 8;
        const x = 18 + i * 36;
        const y = 90 - barH;
        ...
        return (
          <g key={day}>
            <rect x={x} y={y} width={22} height={barH} rx={5} fill={isToday ? "#7AB840" : "#E4EDCC"} />
            <text
              x={x + 11}
              y={104}
              textAnchor="middle"
            ...
            {isToday && counts[i] > 0 && (
              <g>
                <circle cx={x + 11} cy={y - 11} r={11} fill="#1A1A1A" />
                <text
                  x={x + 11}
                  y={y - 7}
                  textAnchor="middle"
    ...
    <div className="flex flex-col h-full bg-[#F4F4E4] overflow-y-auto">
      <div className="px-5 pt-6 pb-8">
        <div className="flex items-center justify-between mb-7">
          ...
          <div className="w-11 h-11 bg-[#7AB840] rounded-full flex items-center justify-center text-white font-bold text-base">
            {initial}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="bg-white rounded-2xl p-4">
            <p className="text-xs text-[#9A9A9A] mb-1">Emails received</p>
            <p className="text-3xl font-bold text-[#1A1A1A]">{messages.length}</p>
            <p className="text-xs text-[#7AB840] mt-1 font-medium">+{todayCount} today</p>
            <div className="mt-3 w-8 h-8 bg-[#EBF4D8] rounded-full flex items-center justify-center">
              <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
                <path
                  d="M2 11 L5.5 6.5 L8 9 L10.5 5.5 L13 11"
                  stroke="#7AB840"
                  strokeWidth="1.6"
                  strokeLinecap="round"
          ...
          <div className="bg-white rounded-2xl p-4">
            <p className="text-xs text-[#9A9A9A] mb-1">Active emails</p>
            <p className="text-3xl font-bold text-[#1A1A1A]">{mailbox ? 1 : 0}</p>
            <p className="text-xs text-[#7AB840] mt-1 font-medium">+{mailbox ? 1 : 0} today</p>
            <div className="mt-3 w-8 h-8 bg-[#EBF4D8] rounded-full flex items-center justify-center">
              <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
                <path
                  d="M1.5 4 L7.5 9 L13.5 4 M1.5 4 H13.5 V12 H1.5 V4Z"
                  stroke="#7AB840"
                  strokeWidth="1.4"
                  strokeLinecap="round"
                  strokeLinejoin="round"
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="w-10 h-10 bg-[#FBF5D8] rounded-full flex items-center justify-center flex-shrink-0">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                <path
                  d="M9 2a5 5 0 0 1 3.8 8.2L12 12H6l-.8-1.8A5 5 0 0 1 9 2Z"
                  stroke="#E8A820"
                  strokeWidth="1.4"
                  strokeLinejoin="round"
                />
